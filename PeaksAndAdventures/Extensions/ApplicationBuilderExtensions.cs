...
namespace Microsoft.AspNetCore.Builder
{
	public static class ApplicationBuilderExtensions
	{
		public static async Task ConfigureRolesAndAdmin(this IApplicationBuilder app)
		{
			...
			var mountainGuide = await userManager.FindByEmailAsync(MountaineerEmail);
			if (mountainGuide != null)
			{
				await userManager.AddToRoleAsync(mountainGuide, MountaineerRole);
			}

			var amateurMountaineer = await userManager.FindByEmailAsync(AmateurMountaineerEmail);
			if (amateurMountaineer != null)
			{
				await userManager.AddToRoleAsync(amateurMountaineer, AmateurMountaineerRole);
			}

			var tourAgency = await userManager.FindByEmailAsync(TourAgencyEmail);
			if (tourAgency != null)
			{
				await userManager.AddToRoleAsync(tourAgency, TourAgencyRole);
			}
