...
namespace PeaksAndAdventures.Infrastructure.Data.Configurations
{
    public class RouteLakeConfiguration : IEntityTypeConfiguration<RouteLake>
    {
        public void Configure(EntityTypeBuilder<RouteLake> builder)
        {
            ...
            //builder.HasData(GenerateRoutesLakes());
        }

        private RouteLake[] GenerateRoutesLakes()
        {
            ICollection<RouteLake> routesLakes = new HashSet<RouteLake>();

            routesLakes.Add(new RouteLake()
            {
                RouteId = 4,
                LakeId = 3,
            });

            return routesLakes.ToArray();
        }
    }
}
